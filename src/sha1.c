...
/* ==== MACROS RECEIVED FROM configure START (change carefully) ==== */
...
#endif
/* ==== MACROS RECEIVED FROM configure END ==== */
...
void SHA1(const uint8_t* str, const uint64_t len, uint8_t ret[20]) {
	...
	for (xblock = 0; xblock < block_count; xblock++) {
		/*memset(W, 0x00, 80 * sizeof(uint32_t)); useless - 0-16 are filled by the M to W converter and 17-79 from Method 1 Step B so all are overwritten *//* Clear the W */
		/* Method 1 Step A */
...
void SHA1_Update(SHA1_CTX* context, const uint8_t* data, const uint64_t ilen) {
	uint64_t len = ilen; /* Need non constant len variable - will changed to be able to work with const */
